...
Consider a chain $X_1\to X_2\to\cdots\to X_n$ where we compute $P(X_1\mid X_n=T)$.

\paragraph{Enumeration.}
We must sum over all $2^{n-2}$ joint assignments to $X_2,\ldots,X_{n-1}$.
Each such assignment requires $n$ multiplications (one per factor in the chain), so the
total cost is
\[
O\!\left(n\cdot 2^{n-2}\right)=O(n\cdot 2^n),
\]
which is \textbf{exponential} in $n$. More concretely, for $n$ variables there are $2^{n-2}$
hidden-variable assignments, and each requires $(n-1)$ multiplications plus one for the
evidence factor, giving approximately $n\cdot 2^{n-2}$ multiplications and $2^{n-2}-1$
additions in the summation.

\paragraph{Variable elimination.}
We eliminate variables right-to-left. Starting from the rightmost hidden variable $X_{n-1}$:
\[
...
\begin{itemize}
  \item 2 multiplications (one per value of $x_{n-1}$),
  \item 1 addition, and
  \item produces a factor $f_{X_{n-1}}$ over the single variable $x_{n-2}$.
\end{itemize}
Each of the $n-2$ elimination steps has the same cost, so the total number of arithmetic
operations is:
\[
(n-2)\times(2\text{ multiplications}+1\text{ addition})\;+\;1\text{ normalisation division}
=O(n),
\]
which is \textbf{linear} in $n$ --- exponentially faster than enumeration.
...
\[
\propto\langle0,0.8,0,0,0,0\rangle\xrightarrow{\times1.25}\langle0,1,0,0,0,0\rangle
\]
\[
...
\[
\boxed{V^{\pi}(s)=R\bigl(s,\pi(s)\bigr)+\sum_{s'}T\bigl(s,\pi(s),s'\bigr)\,V^{\pi}(s')}
\]
...
\end{align*}
Since $4>0$, set $\boxed{\pi_{\text{new}}(A)=2}$.
...
\end{align*}
Since $15>10$, set $\boxed{\pi_{\text{new}}(B)=1}$.
