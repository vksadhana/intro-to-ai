...
def single_astar_backward(agent_pos, goal, known_blocked):
    ...
    h_start = manhattan(goal, agent_pos)
    f_start = g_values[goal] + h_start
    open_list.push(f_start, g_values[goal], goal)
    ...
    while open_list.is_empty() == False:
        ...
        for neighbor in all_neighbors:
            ...
            if neighbor not in g_values or new_g < g_values[neighbor]:
                ...
                h_val = manhattan(neighbor, agent_pos)
                f_val = new_g + h_val
...
def repeated_backward_astar(
    ...
    while current_pos != goal:
        path, num_exp = single_astar_backward(current_pos, goal, known_blocked)
...
def show_astar_search(win: pygame.Surface, actual_maze: List[List[int]], algo: str, fps: int = 240, step_delay_ms: int = 0, save_path: Optional[str] = None) -> None:
    ...
    while current_pos != goal and keep_going == True:
        ...
        path, exp = single_astar_backward(current_pos, goal, known_blocked)
        if path is None:
            break
